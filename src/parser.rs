...
impl<'a, T: Read> Parser<T> {
    ...
    pub fn parse_value(&mut self) -> RedisResult<Value> {
        ...
        match b as char {
            ...
            '*' => self.parse_bulk(),
            '-' => self.parse_error(),
            _ => fail!((ErrorKind::PatternError, "Invalid response when parsing value")),
        }
    }
    ...
    fn expect_char(&mut self, refchar: char) -> RedisResult<()> {
        if try!(self.read_byte()) as char == refchar {
            ...
            fail!((ErrorKind::PatternError, "Invalid byte in response"));
        }
    }
    ...
    fn expect_newline(&mut self) -> RedisResult<()> {
        match try!(self.read_byte()) as char {
            '\r' => self.expect_char('\n'),
            _ => fail!((ErrorKind::PatternError, "Invalid byte in response")),
        }
    }
    ...
    fn read_string_line(&mut self) -> RedisResult<String> {
        match String::from_utf8(try!(self.read_line())) {
            Err(_) => fail!((ErrorKind::PatternError, "Expected valid string, got garbage")),
            Ok(value) => Ok(value),
        }
    }
    ...
    fn read_byte(&mut self) -> RedisResult<u8> {
        ...
        if nread < 1 {
            fail!((ErrorKind::PatternError, "Could not read enough bytes"))
        } else {
            Ok(buf[0])
        }
    }
    ...
    fn read(&mut self, bytes: usize) -> RedisResult<Vec<u8>> {
        ...
        while i < bytes {
            ...
            match res_nread {
                ...
                Ok(_) => return fail!((ErrorKind::PatternError, "Could not read enough bytes")),
    ...
    fn read_int_line(&mut self) -> RedisResult<i64> {
        ...
        match line.trim().parse::<i64>() {
            Err(_) => fail!((ErrorKind::PatternError, "Expected integer, got garbage")),
            Ok(value) => Ok(value),
        }
    }
