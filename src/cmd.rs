...
impl Cmd {
    ...
    #[inline]
    pub fn query_cluster<T: FromRedisValue>(&self, cluster: &mut Cluster) -> RedisResult<T> {
        cluster.query_cmd(self)
    }
...
/// # let client = td_rredis::Client::open("redis://127.0.0.1/").unwrap();
/// # let con = client.get_connection().unwrap();
/// let ((k1, k2),) : ((i32, i32),) = td_rredis::pipe()
...
impl Pipeline {
    ...
    /// # let client = td_rredis::Client::open("redis://127.0.0.1/").unwrap();
    /// # let con = client.get_connection().unwrap();
    /// let _ : () = td_rredis::pipe().cmd("PING").query(&con).unwrap();
    /// ```
    #[inline]
    pub fn execute(&self, con: &ConnectionLike) {
        let _: () = self.query(con).unwrap();
    ...
    pub fn get_slot(&self) -> u16 {
        if self.commands.len() > 0 {
            return self.commands[1].get_slot();
        }
        0
    }
}
...
/// let cmd = td_rredis::pack_command(&args);
/// assert_eq!(cmd, b"*3\r\n$3\r\nSET\r\n$6\r\nmy_key\r\n$2\r\n42\r\n".to_vec());
/// ```
